import { href, type Tab } from '../router.ts'

export function TabBar({ current }: { current: Tab }) {
  const tab = (name: Tab) => (name === current ? { 'aria-current': 'page' as const } : {})
  return (
    <nav className="tabs" aria-label="Sections">
      <a href={href({ name: 'games' })} {...tab('games')}>
      ...
      <a href={href({ name: 'players' })} {...tab('players')}>
      ...
      <a href={href({ name: 'stats' })} {...tab('stats')}>
